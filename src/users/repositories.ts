...
export class UsersRepository {
  ...
  async getByIdWithRelations(
    ...
    const albumsOptions: Prisma.Profile$albumsArgs = {
      include: { images: { include: { image: true } }, topic: true },
    };
    if (shownOnly) {
  ...
  async getFriendsCountForUser(userId: number) {
    return await prisma.friendship.count({
      where: {
        OR: [{ profile1_id: userId }, { profile2_id: userId }],
        accepted: true,
      },
    });
  }
  ...
  async getAllWithoutFriendshipWith(userId: number) {
    return await prisma.user.findMany({
      where: {
        profile: {
          friendship_sent_request: { none: { profile2_id: userId } },
          friendship_accepted_request: { none: { profile1_id: userId } },
        },
      },
      include: { profile: { include: { avatars: true } } },
    });
  ...
  async updateById(userId: number, data: updateMeInput): Promise<User> {
    ...
    const updateInput: Prisma.UserUpdateInput = {
      ...{ ...data, avatarUrl: undefined, date_of_birth: undefined },
    ...
    return prisma.friendship.update({
      where: {
        profile1_id_profile2_id: {
          profile1_id: fromUserId,
          profile2_id: toUserId,
        },
      },
      data: { accepted: true },
    });
    ...
    try {
      return await prisma.album.update({
        ...
        data: {
          ...data,
          images: {
            create: data.images.map((el) => ({ image: { create: el } })),
          },
        },
      });
    } catch (err) {
